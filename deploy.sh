...
echo "----------------------------------------"
if [ ! -f "gcs-lifecycle.json" ]; then
  echo "⚠️ gcs-lifecycle.json 파일을 찾을 수 없습니다. Lifecycle 정책 적용을 건너뜁니다."
elif $GCLOUD_PATH storage buckets update "gs://$GCS_BUCKET_NAME" \
  --project="$PROJECT_ID" \
  --lifecycle-file=gcs-lifecycle.json; then
