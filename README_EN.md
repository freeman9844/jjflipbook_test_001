...
*   **Android Page-Flip Flickering Fix**: Identified a GPU compositing conflict on Android Chrome where `react-pageflip` auto-injects `transform: translateZ(0)` onto `.stf__parent`, creating a GPU layer that clashes with the outer scale wrapper's `transform: scale()` layer. Fixed by overriding the library's translateZ to `none !important` in `globals.css`, and adding `will-change: transform` + `backface-visibility: hidden` to unify both into a single GPU composite layer. Additionally removed `backdrop-filter: blur()` on mobile (reduces GPU load) and set `mobileScrollSupport={false}` to prevent Android touch event conflicts during page flipping.
